...
test("parseEnvFile reads plain and quoted values", () => {
  ...
  assert.equal(parsed.OUTBOUND_TARGET_E164, "+15555550100");
...
test("applyEnvUpdates replaces existing keys and appends missing ones", () => {
  const out = applyEnvUpdates(
    ["OUTBOUND_TARGET_E164=+111", "ASTERISK_SHARED_SECRET=old", ""].join("\n"),
    {
      OUTBOUND_TARGET_E164: "+222",
      TWILIO_AUTH_TOKEN: "abc123",
    },
  );

  assert.equal(out.includes("OUTBOUND_TARGET_E164=+222"), true);
