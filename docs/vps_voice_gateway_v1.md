...
Date: 2026-01-31
Owner: Levi
Scope: Always-on VPS voice gateway with SIP (VoIP.ms) + Twilio media streams, streaming STT/MT/TTS via cloud APIs, EN↔ES. Private translation mix injected into the outbound Telcel call leg by default; relay to vendor optional (v1.2+). Inbound calls auto-ring the Telcel number.
...
- Private translation mix on outbound Telcel call leg by default.
- Pluggable providers for STT/MT/TTS.

## Default Behavior (v1)
- Inbound calls to VoIP.ms or Twilio auto-ring the Telcel number.
- Translation is ON by default on the Telcel call leg.
...
Outputs:
- Private mix (default): original + translated (Telcel call leg)
...
- Configure basic dialplan for:
  - inbound SIP -> audio bus -> outbound Telcel leg
...
- On inbound, Asterisk originates an outbound Telcel leg and injects private translation mix.
...
- On inbound, bridge to outbound Telcel leg and inject private translation mix.

### 4) Voice Gateway Service
- Outbound leg: auto-call Telcel on inbound (E.164 from config).
- HTTP endpoints:
  - /health
  - /twilio/voice (TwiML)
  - /twilio/stream (WS)
...
- Mix outputs:
  - Default: private mix on outbound Telcel call leg (stereo split when available)
...
### Telcel Number (config)
- Store E.164 in config, e.g. `+52XXXXXXXXXX`.

---

## Acceptance Tests (v1)
- Inbound VoIP.ms call -> VPS auto-rings Telcel -> translated audio in call leg within <1s for short phrases.
- Inbound Twilio call -> Media Stream -> VPS auto-rings Telcel -> translated audio in call leg.
